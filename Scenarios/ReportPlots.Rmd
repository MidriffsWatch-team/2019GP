...
res<-read.csv(here::here("Scenarios", "PatchModel_size.csv"))%>%
  filter(Year <= 2065)%>%
  filter(Total_Area=="BAU" |Total_Area=="5%" | Total_Area=="20%" | Total_Area=="40%" | Total_Area=="50%")%>%
...
KobeOA_inflated
ggsave("KobeOA_inflated.jpg", width=12, height=10, dpi=300)
...
initial <- res%>%
  filter(Year == 2015 & Scenario == 0 & Size=="5%")%>%
  mutate(Scenario = "initial")

plot<-fmsy%>%
  filter(Year== 2050 &  Size == "5%")%>%
  rbind(initial)
...
ggsave("Kobe2.jpg", width=8, height=5, dpi=300)
