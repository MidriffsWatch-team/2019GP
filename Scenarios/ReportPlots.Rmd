...
```{r}
initial <- res%>%
  filter(Year == 2015 & Adjusted == "IUU_0" & Reserve_size=="BAU")%>%
  mutate(Implementation_year = "Initial")

plot <- ggplot(initial, aes(x= bbmsy, y=ffmsy))+
  ...
  geom_point(aes(size=Catch_est), show.legend = FALSE)+
...
```{r}
initial <- res%>%
  filter(Year == 2015 & Implementation_year == "BAU" & Adjusted=="IUU_0")%>%
  mutate(Implementation_year = "Initial")
...
```{r}
initial <- res%>%
  filter(Year == 2015 & Implementation_year == "BAU")%>%
  filter(Adjusted == "IUU_0" | Adjusted == "IUU_20" |Adjusted == "IUU_40" |Adjusted == "IUU_60")%>%
  mutate(Implementation_year = "Initial")
...
KobeOA <- ggplot(plot, aes(x= bbmsy, y=ffmsy, group=Name))+
  ...
  scale_shape_manual(values=c(16, 17, 15, 18, 25, 20))+
  scale_colour_brewer(palette = "pal6")+
  labs(x=bmsy, y=fmsy)+
  geom_vline(xintercept = 1, linetype="dotted", color="black") +
  geom_hline(yintercept = 1, linetype="dotted", color="black") +
  #scale_colour_discrete(name="Implementation year")+
...
OA <-res%>%
  filter(Implementation_year=="2015")

KobeOA_size<-ggplot(OA, aes(x= bbmsy, y=ffmsy, group=Name))+
  geom_point(size=2, aes(color=Year, shape=Reserve_size))+
...
plot<-res%>%
  filter(Year== 2065, Adjusted== "IUU_0")
plot$Implementation_year<- as.factor(plot$Implementation_year)
...
#plot2$Implementation_year <- factor(plot2$Size, levels = c("BAU", "5%", "20%", "40%"))
