...
## Version modified by Alfredo Giron-Nava to be used with Giron-Nava et al. (2018)
##---------------------------------------------------------------------------------------------
...
setwd('/Users/alfredogiron/Dropbox/Papers_Alfredo/Papers2017/MSY_alfredo')
#-----------------------------------------
...
#-----------------------------------------
catch_file  <-  "data/data_conapesca_MSY.csv" #  name of file containing "stock", "yr", "ct", and optional "bt"
id_file     <-  "data/metadata_conapesca_MSY.csv"  #  name of file containing stock-specific info and settings for the analysis
...
cdat$ct = cdat$ct/1000 # Transform catch to tonnes
...
for (i in c(50,55,64,78,89,101)){#1:nrow(cinfo)) {
  stock = stocks[i]
  cat("Processing",stock,",", as.character(cinfo$Group[i]))
  # assign data from cinfo to vectors
  res          <- as.character(cinfo$Resilience[i])
  start.yr     <- as.numeric(cinfo$StartYear[i])
  end.yr       <- as.numeric(cinfo$EndYear[i])
  r.low        <- as.numeric(cinfo$r.low[i])
  r.hi         <- as.numeric(cinfo$r.hi[i])
  user.log.r   <- ifelse(is.na(r.low)==F & is.na(r.hi)==F,TRUE,FALSE)     
  stb.low      <- as.numeric(cinfo$stb.low[i])
  stb.hi       <- as.numeric(cinfo$stb.hi[i])
  int.yr       <- as.numeric(cinfo$int.yr[i])
  intb.low     <- as.numeric(cinfo$intb.low[i])
  intb.hi      <- as.numeric(cinfo$intb.hi[i])
  endb.low     <- as.numeric(cinfo$endb.low[i])
  endb.hi      <- as.numeric(cinfo$endb.hi[i])
  btype        <- as.character(cinfo$btype[i])
  force.cmsy   <- ifelse(force.cmsy==T,T,cinfo$force.cmsy[i])
  comment      <- as.character(cinfo$Comment[i])
  ...
  ct.raw           <- as.numeric(cdat$ct[cdat$Stock==stock & cdat$yr >= start.yr & cdat$yr <= end.yr])/1000  ## assumes that catch is given in kg, transforms to  tonnes
  ...
  plot(x=yr, y=ct.raw, 
       ylim=c(0,max(ifelse(substr(id_file,1,3)=="Sim",
                           1.1*cinfo$true.MSY/1000,0),1.2*max(ct.raw))),
  ...
  if(btype != "None" & length(bt[is.na(bt)==F])>=nab) {
    ...
    q.prior         <- c(q.1,q.2) ### You need to check this one!!!
    init.q          <- mean(q.prior)
  ...
  #-------------------------------------------
  cat("---------------------------------------\n")
  cat("Species:", cinfo$ScientificName[i], ", stock:",stock,"\n")
  cat(cinfo$Group[i], "\n")
  cat("Region:",cinfo$Region[i],",",cinfo$Subregion[i],"\n")
  ...
  max.y    <- max(c(ifelse(FullSchaefer==T,ucl.k.jags,NA), max(kv.all)), 
                  ifelse(substr(id_file,1,3)=="Sim",1.2*cinfo$true.k,max(kv.all)),    
                  na.rm=T)
  min.y    <- min(c(ifelse(FullSchaefer==T,lcl.k.jags,NA), 0.9*min(kv.all)), 
                  ifelse(substr(id_file,1,3)=="Sim",0.8*cinfo$true.k,0.9*min(kv.all)),    
                  na.rm=T)
  ...
  if (save.plots==TRUE) 
  {
    jpgfile<-paste("output/figures/froese/",stock,".jpeg",sep="")
  ...
  {
    jpgfile<-paste("output/figures/froese/",stock,"_MAN.jpeg",sep="")
    dev.copy(jpeg,jpgfile,
             width = 1024, 
             height = 768, 
             units = "px", 
  ...
  if(write.output == TRUE) {
    ...
    output = data.frame(as.character(cinfo$Group[i]),
                        as.character(cinfo$Region[i]),
                        as.character(cinfo$Subregion[i]),
                        as.character(cinfo$Name[i]),
                        cinfo$ScientificName[i], 
    ...
    # ifelse(yr[1]>2000,NA,ct.raw[yr==2000]),ifelse(yr[1]>2001,NA,ct.raw[yr==2001]),
    # ifelse(yr[1]>2003,NA,ct.raw[yr==2003]),# allow missing 2000-2002
    ...
    cat("Species:", cinfo$ScientificName[i], ", stock:",stock,"\n",
        cinfo$Name[i], "\n",
        "Source:",cinfo$Source[i],"\n",
        "Region:",cinfo$Region[i],",",cinfo$Subregion[i],"\n",
  ...
  result_species = data.frame(cinfo$Name[i],cinfo$Subregion[i],stock,yr,ct.raw,B,lcl.B,ucl.B,B.Bmsy,lcl.B.Bmsy,ucl.B.Bmsy,F.Fmsy,lcl.F.Fmsy,ucl.F.Fmsy)
  RESULT_TS = rbind(RESULT_TS,result_species)
  } # end of stocks loop
