...
landings<- catch%>%
  group_by(Ano)%>%
  summarize(weight=sum(PesoDesembarcado),
            value=sum(Valor))%>%
...
#Load species from Stuart 15/05/18

```{r}
species <- read_csv("G:/GitHub/2019GP/raw_data/species.csv")
...
```{r}
value<- catch%>%
  select(NombreCientifico, NombrePrincipal, Mes, Ano, PesoDesembarcado, Precio)

spp<- species%>%
  select(NombreCientifico, PrecioMinimo, PrecioMaximo, promedio)


# 42 of the species Stuart sent us are found in the CONAPESCA database 
merged<-merge(value, spp, by= "NombreCientifico")%>%
   mutate(minvalue=(PesoDesembarcado*PrecioMinimo))%>%
  mutate(avgvalue=(PesoDesembarcado* promedio))%>%
   mutate(maxvalue=(PesoDesembarcado*PrecioMaximo))%>%
  as.data.frame()
...
write.csv(ranked, file="ranked.csv")
