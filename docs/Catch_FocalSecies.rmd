...
#Load Catch data and list of 17 focal species
...
=======
#test
>>>>>>> 6f747e215632c0d378f870e38e8a38fbca06d04c
...
graph1<-ggplot(allcatch, aes(x=Ano))+
  ...
  labs(x="Year", y="Yearly total landings (MT)", title="Yearly total reported catch in Baja California Sur and Sonora", subtitle= "Small scale landings")+
    theme_classic()
...
graph2<-ggplot(allcatch, aes(x=Ano))+
  ...
  labs(x="Year", y="Number of species landed", title="Number of species landed in Baja California Sur and Sonora")+
    theme_classic()
...
#For 17 focal species calculate catch per species per year
...
graph5 <-ggplot(catch_yr, aes(x=Ano, y=MT, color=NombreCientifico))+
  ...
  geom_line(size=1)+
  geom_point()+
  labs(x="Year", y="Yearly total catch catch (MT)", title="Yearly Catch of 17 focal species", subtitle= "2000-2015")+
  theme_classic()+
...
```{r, echo=FALSE}
plot_ly(value2014, labels = ~CommonName, values = ~total_avg_value_2014, type = 'pie', showlegend=FALSE) %>%
  layout(title = 'Revenue generated by 17 focal species in 2014 in USD')

```


```{r, echo=FALSE}
plot_ly(value2014, labels = ~CommonName, values = ~MT, type = 'pie', showlegend=FALSE) %>%
