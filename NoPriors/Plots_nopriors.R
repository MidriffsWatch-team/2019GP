...
############################################################
nopriors <- readRDS(here::here("NoPriors", "PatchModel_size_nopriors.rds"))%>%
  filter(Year<=2065)
...
ggsave("Line2_nopriors.jpg", width=4, height=4, dpi=300)
...
ggsave("Biomass_Fishery_nopriors.jpg", width=5, height=4, dpi=300)
...
C<-ggplot(Fisheries, aes(x=Year, y=Catch_est, group= Name))+
  ...
  theme_classic(base_size = 9)+
  facet_wrap(~ Name, scales="free" , ncol=3)
ggsave("Catch_Fishery_nopriors.jpg", width=5, height=4, dpi=300)
...
Fishing <-ggplot(Fisheries, aes(x=Year, y=ffmsy, group= Name))+
  ...
  theme_classic(base_size = 9)+
  facet_wrap(~ Name, scales="free" , ncol=3)
ggsave("F_Fishery_nopriors.jpg", width=5, height=4, dpi=300)
...
initial <- read.csv(here::here("NoPriors","Genus_pts_nopriors.csv"))%>%
  filter(Adjusted=="IUU_0")%>%
  filter(!(Name=="Caranx" |Name=="Sphoeroides"))
...
ggsave("Kobe2015_label_nopriors.jpg", width=4, height=3, dpi=300)
...
KobeOA_size
ggsave("Kobe_byMRSize_nopriors.jpg", width=6, height=4, dpi=300)